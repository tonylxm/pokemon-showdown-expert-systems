...
class CustomAgent(Player):   
    ...
    def choose_move(self, battle: AbstractBattle):
        ...
        # Logging is handled inside _select_action for moves; for switches or fallback, log here
    ...
    def _select_action(self, battle: AbstractBattle, strategy: str):
        """Select specific action based on strategy with meta-aware enhancements"""
        ...
        if battle.active_pokemon and hasattr(battle, 'available_moves') and battle.available_moves:
            ...
            for move, priority, reasoning in move_evaluations:
                if move.id in hazard_moves:
                    hazard_score, hazard_reason = AdvancedBattleStrategy.evaluate_hazard_priority(battle)
                    if hazard_score * 15 + priority > best_priority:
                        best_move = move
                        best_priority = hazard_score * 15 + priority
                        best_reasoning = f"Hazard stacking: {hazard_reason}"
            
            # Endgame priority moves
            ...
            if alive_count <= 2 or opp_alive <= 2:
                if best_move.id.lower() in ["extremespeed", "suckerpunch", "bulletpunch"]:
                    best_priority += 30
                    best_reasoning += " | Endgame priority move"

            # Predictive switching
            predictions = AdvancedBattleStrategy.predict_opponent_move(battle)
            if "switch" in predictions and predictions["switch"] > 0.6:
                for pokemon_name, pokemon in battle.team.items():
                    if pokemon != battle.active_pokemon and not pokemon.fainted:
                        opp_types = [t.name if hasattr(t, 'name') else str(t) for t in battle.opponent_active_pokemon.types]
                        poke_types = [t.name if hasattr(t, 'name') else str(t) for t in pokemon.types]
                        for opp_type in opp_types:
                            if PokemonKnowledge.get_type_effectiveness(opp_type, poke_types) <= 0.5:
                                return self.create_order(pokemon)
            
            # Log reasoning for debugging
            self._log_decision_advanced(battle, best_move, best_reasoning)
            return self.create_order(best_move)
        
        # Fallback
        return self.choose_random_move(battle)
    
    def _log_decision_advanced(self, battle: AbstractBattle, move, reasoning: str):
        """Log advanced meta reasoning"""
        record = {
