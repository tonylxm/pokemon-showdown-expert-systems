...
class CustomAgent(Player):   
    def __init__(self, *args, **kwargs):
        ...
        self.hazard_turns = 2  # prioritize hazard stacking first 2 turns
        self.taunt_targets = ["deoxys-speed", "arceus", "calyrex-shadow"]
    ...
    def _select_action(self, battle: AbstractBattle, strategy: str):
        ...
        if battle.active_pokemon and hasattr(battle, 'available_moves') and battle.available_moves:
            ...
            # Early game Taunt vs setup sweepers
            for move, priority, reasoning in move_evaluations:
                if move.id == "taunt" and battle.turn <= 3:
                    opp_name = battle.opponent_active_pokemon.species.lower()
                    if opp_name in self.taunt_targets:
                        priority += 40
                        reasoning += " | Early Taunt to prevent setup"

            # Hazard stacking priority
            if battle.turn <= self.hazard_turns:
                for move, priority, reasoning in move_evaluations:
                    if move.id in ["spikes", "stealthrock", "toxicspikes"]:
                        priority += 25
                        reasoning += " | Early hazard setup"
